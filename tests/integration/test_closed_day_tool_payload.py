...
def _tools(open_days: tuple[int, ...] = MON_TO_SAT) -> dict[str, Any]:
    ...
    # Monday the 14th onward is published; Sunday the 13th is not, and never will be.
    appointments.seed_slots(
        [
            _slot("mon-0900", "2026-09-14T09:00"),
            _slot("mon-0930", "2026-09-14T09:30"),
            _slot("tue-0900", "2026-09-15T09:00"),
        ]
    )
...
def test_the_notice_calls_it_the_clinic_holiday() -> None:
    ...
    notice = _availability(_tools(), "2026-09-13")["closed_notice"]
...
def test_slots_are_still_offered_so_the_caller_gets_an_alternative() -> None:
    ...
    assert all(slot.start[:10] != "2026-09-13" for slot in payload["value"])
...
def test_an_open_day_carries_no_closure_flag() -> None:
    payload = _availability(_tools(), "2026-09-15")
...
def test_a_clinic_open_on_sunday_carries_no_closure_flag() -> None:
    ...
    payload = _availability(_tools(open_days=(0, 1, 2, 3, 4, 5, 6)), "2026-09-13")
    assert "clinic_closed_on_requested_date" not in payload
